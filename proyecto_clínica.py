...
################################################### Tabla ###############################################################
tree = ttk.Treeview(height=10, columns=('#0', '#1', '#2', '#3','#4'))# En el primer parámetro de 'Treeview' (height) le damos altura, en el segundo (columns) le damos la cantidad de columnas con numeral.
tree.place(x=0, y=130)
tree.column('#0', width=100) # Especificamos la columna y le damos una anchura de 100
tree.heading('#0', text='ID', anchor=tk.CENTER) # Especificando de nuevo la columna, le adquirimos en el parametro 'text' el nombre, y con 'anchor' le decimos que la palabra quede centrada.
...
# Actualizar - Sirve para cuando queremos actualizar los datos de alguien
def actualizar():
    conn = sqlite3.connect('base_pacientes')
    cursor = conn.cursor()
    try:
